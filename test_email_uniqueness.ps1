...
Write-Host "`n📧 Test 1: Checking email availability for 'test@example.com'" -ForegroundColor Yellow
try {
...
Write-Host "`n👤 Test 2: Creating first user with email 'test@example.com'" -ForegroundColor Yellow
$user1 = @{
    username = "testuser1"
...
Write-Host "`n🚫 Test 3: Trying to create second user with same email 'test@example.com'" -ForegroundColor Yellow
$user2 = @{
    username = "testuser2"
...
Write-Host "`n📧 Test 4: Checking email availability for 'test@example.com' (should be false now)" -ForegroundColor Yellow
try {
    $response = Invoke-RestMethod -Uri "$baseUrl/check-email?email=test@example.com" -Method Get
...
Write-Host "`n👤 Test 5: Creating user with different email 'another@example.com'" -ForegroundColor Yellow
$user3 = @{
    username = "testuser3"
...
Write-Host "`n🔄 Test 6: Trying to update first user's email to 'another@example.com' (should fail)" -ForegroundColor Yellow
$updateUser = @{
    username = "testuser1"
...
Write-Host "`n🔧 Test 7: Trying to patch update with existing email 'another@example.com' (should fail)" -ForegroundColor Yellow
$patchUpdate = @{
    email = "another@example.com"  # Email already exists!
} | ConvertTo-Json
...
Write-Host "`n🧹 Test 8: Cleaning up test users" -ForegroundColor Yellow
try {
    if ($userId1) {
...
Write-Host "`n🎉 Email Uniqueness Testing Complete!" -ForegroundColor Green
Write-Host "=====================================" -ForegroundColor Green
